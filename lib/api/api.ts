...
export const handleApiBackendError = (error: unknown): ApiError => {
    ...
    if (axios.isAxiosError(error)) {
        ...
        if (responseData?.data?.error) {
            ...
            return new ApiError({
                statusCode: status,
                messages: invalidMessages.length > 0 ? invalidMessages : ["Validation failed"],
        ...
        if (errorObj.details && typeof errorObj.details === "object") {
            ...
            Object.entries(errorObj.details).forEach(([field, fieldMessages]) => {
                if (Array.isArray(fieldMessages)) {
                    fieldMessages.forEach((msg) => invalidMessages.push(`${field}: ${msg}`));
...
> => {
  ...
  try {
    ...
    return {
      success: data.message || "Activity Performed Successfully",
