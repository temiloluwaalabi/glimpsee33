...
export const useFeedState = () => {
    ...
    useEffect(() => {
        if(!isInitialized.current){
            const urlParams: Record<string, string> = {
                ...
                sortBy: ["newest", "oldest", "popular", "trending"].includes(feedState.sortBy)
                    ? feedState.sortBy
                    : "newest",
    ...
    }, [feedState.category, feedState.gSearch, feedState.page, feedState.search, feedState.sortBy, feedState.viewMode, store, storeStateKey, urlStateKey])
  ...
  // Sync URL changes to store (when user navigates with browser back/forward)
    ...
    useEffect(() => {
        ...
        if(urlStateKey !== lastURLUpdate.current && urlStateKey !== lastStoreUpdate.current){
            store.updateFilters({
    ...
    }, [feedState.category, feedState.gSearch, feedState.page, feedState.search, feedState.sortBy, feedState.viewMode, store, urlStateKey])
    ...
    if (storeStateKey !== lastStoreUpdate.current && storeStateKey !== lastURLUpdate.current) {
      updateFeedState({
          search: store.filters.search,
        globalSearch: store.filters.globalSearch,
        category: store.filters.category,
...
const actions = {
    setglobalSearchQuery: useCallback((query: string) => {
        store.setGlobalSearchQuery(query)
    }, [store]),
    setLocalSearchQuery: useCallback((query: string) => {
    ...
    setViewMode: useCallback((view: 'grid' | 'list') => {
    ...
    batchUpdate: useCallback((updates: Partial<FeedQuery>) => {
...
export const useFeedSearch = () => {
    const {filters, setLocalSearchQuery, setglobalSearchQuery, cache} = useFeedState()
...
export const useFeedFilters = () => {
  const { filters, setSortBy, setSelectedCategory, resetFilters, activeFiltersCount } = useFeedState();
